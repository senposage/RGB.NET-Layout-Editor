...
public sealed class OpenRgbClient : IDisposable, IOpenRgbClient
{
    ...
    {
        _controllerIds = _connection.Request<EmptyArg, ControllerIdsReader, uint[]>(CommandId.RequestControllerCount, 0, new EmptyArg());
        return _controllerIds.Length;
    }
    ...
    public Device GetControllerData(int deviceId)
    {
        ...
        var controllerId = GetControllerId(deviceId);
        return _connection.Request<ProtocolVersionArg, DeviceReader, Device>(CommandId.RequestControllerData, controllerId,
    ...
    {
        _connection.Send(CommandId.ResizeZone, GetControllerId(deviceId), new Args<uint, uint>((uint)zoneId, (uint)size));
    }
    ...
    public void UpdateLeds(int deviceId, ReadOnlySpan<Color> colors)
    {
        ...
        _connection.Send(CommandId.UpdateLeds, GetControllerId(deviceId), new UpdateLedsArg((ushort)colors.Length), bytes);
    }
    ...
    public void UpdateZoneLeds(int deviceId, int zoneId, ReadOnlySpan<Color> colors)
    {
        ...
        _connection.Send(CommandId.UpdateZoneLeds, GetControllerId(deviceId), new UpdateZoneLedsArg((uint)zoneId, (ushort)colors.Length), bytes);
    }
    ...
    public void UpdateSingleLed(int deviceId, int ledId, Color color)
    {
        ...
        _connection.Send(CommandId.UpdateSingleLed, GetControllerId(deviceId), new Args<uint, Color>((uint)ledId, color));
    }

    /// <inheritdoc />
    public void SetCustomMode(int deviceId)
    {
        _connection.Send(CommandId.SetCustomMode, GetControllerId(deviceId), new EmptyArg());
    }
        ...
        Color[]? colors = default)
    {
        ...
        _connection.Send(CommandId.UpdateMode, GetControllerId(deviceId), new ModeOperationArg((uint)modeId, new ModeArg(targetMode)));
    }
    ...
    public void SaveMode(int deviceId, int modeId)
    {
        ...
        _connection.Send(CommandId.SaveMode, GetControllerId(deviceId), new ModeOperationArg((uint)modeId, new ModeArg(targetMode)));
    }
    ...
    private uint GetControllerId(int deviceIndex)
    {
        if (deviceIndex < 0)
            throw new ArgumentException("Invalid device index.", nameof(deviceIndex));

        if (CommonProtocolVersion.Number < 6)
            return (uint)deviceIndex;

        if (_controllerIds.Length == 0)
            GetControllerCount();

        if (deviceIndex >= _controllerIds.Length)
