...
internal sealed class OpenRgbConnection : IDisposable
{
    ...
    public void Connect(string name, string ip, int port, int timeoutMs, uint protocolVersionNumber = 4)
    {
        ...
        if (CurrentProtocolVersion.Number >= 6)
            Send(CommandId.SetClientFlags, 0, new Args<uint>(1)); // RGBController API
    }
    ...
    private async Task ReadLoop()
    {
        ...
        while (!_cancellationTokenSource.IsCancellationRequested && _socket.Connected)
        {
            try
            {
                ...
                else if (header.Command != CommandId.Ack &&
                         header.Command != CommandId.SetServerName &&
                         header.Command != CommandId.SetServerFlags &&
                         header.Command != CommandId.DetectionStarted &&
                         header.Command != CommandId.DetectionProgressChanged &&
                         header.Command != CommandId.DetectionComplete &&
                         _pendingRequests.TryGetValue(header.Command, out var pendingRequest))
                {
